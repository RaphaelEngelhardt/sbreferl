"""
Provides an RL agent using OPCT (regression)
...
class OPCTReg:
    ...
    def learn(self, samplesfile, threshold=None, nsamples=None,
        ...
        """
        print("Seed as seen by OPCT: ", self.seed)
        ...
        if "num_trees" not in opct_kwargs:
            print("num_trees not spefified, overriding the default value of "
                  "100 with 1")
        ...
        cvs = cross_val_score(self.tree, X, y, cv=5, scoring="r2")
        return {'cvs': cvs,
    ...
    def print_tree(self, inode=0):
        ...
        if node.left != -1:
            ...
            pnum = np.asrray(node.prototype)
            space = (node.depth - 1) * "  |       "
            if node.depth != 0:
                space += "  |-------"
            print("{space}node={inode}, class: {proto}".format(
                space=space,
                inode=inode,
                proto=pnum)
            )
    ...
    def print_tree_pythonlike(self, inode=0):
        ...
        if node.left != -1:
            ...
            pnum = np.asarray(node.prototype)
            space = (node.depth - 1) * "    "
            if node.depth != 0:
                space += "    "
