"""Provides an RL agent using CART (classification)"""
import numpy as np
import pandas as pd
...
class CART:
    def __init__(self):
        ...
        self.is_solved = None
        self.enc = OrdinalEncoder(categories=[np.arange(self.env.action_space.n)])
    ...
    def learn(self, samplesfile, threshold=None, nsamples=None,
        ...
        """
        Trains the Decision tree DecisionTreeClassifier as implemented
        by scikit-learn
        ...
        :param dtc_kwargs: all other kwargs are passed to
                           DecisionTreeClassifier (max_depth etc.)
        ...
        """
        print("Seed as seen by CART: ", self.seed)
        ...
        # Encode labels (this has to be done in order to have all
        # categories available, not only the ones present in training
        # data)
        self.enc.fit(y)
        y_oe = self.enc.transform(y).astype(int)

        # Fit the model
        self.tree = tree.DecisionTreeClassifier(**dtc_kwargs,
                                                random_state=self.seed)
        self.tree.fit(X, y_oe)
